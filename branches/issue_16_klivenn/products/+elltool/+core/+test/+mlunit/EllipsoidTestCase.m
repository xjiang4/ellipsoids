classdef EllipsoidTestCase < mlunitext.test_case
     ...
     methods
        ...
        function self = testRho(self)
            ...
            testIsRight = (abs(testResRho(1) - 3) < absTol) & (abs(testResRho(2) - 4) < absTol);
            %testResRho = testDirection*testEllCenter + sqrt(testDirection*testShapeMat*testDirection);
            mlunit.assert_equals(testIsRight, 1);
            
            testShapeMat1 = [1 1; 2 2];
            testShapeMat2 = [2 2; 1 2];
            testEllCenter1 = [sqrt(2); 1];
            testEllCenter2 = [2*sqrt(2); 4];
            testDirection = [sqrt(2); 0];
            testEll1 = ellipsoid(testEllCenter1, testShapeMat1*testShapeMat1.');
            testEll2 = ellipsoid(testEllCenter2, testShapeMat2*testShapeMat2.');
            testEllVec = [testEll1 testEll2];
            testResRho = rho(testEllVec, testDirection);
            testIsRight = (abs(testResRho(1) -   4) < absTol) & (abs(testResRho(2) - 8) < absTol);            
            mlunit.assert_equals(testIsRight, 1);
        end
        ...
        function self = testMinksum(self)
            ...
            testEllCenter1 = [1; 0];
            testEllCenter2 = [1; 0];
            testResEllCenter = [2; 0];
            testShapeMat1 = eye(2);
            testShapeMat2 = 4*eye(2);
            testEll1 = ellipsoid(testEllCenter1, testShapeMat1);
            testEll2 = ellipsoid(testEllCenter2, testShapeMat2);
            testResEll = minksum([testEll1 testEll2]);
            for i = 1:201
                testResEllConstr(i) = (testResEll(1:2, i) - testResEllCenter).'*[1/9 0; 0 1/9]*(testResEll(1:2,i) - testResEllCenter) - 1;
            end
            testIsRight = min(abs(testResEllConstr(1:201)) < absTol);
            mlunit.assert_equals(1, testIsRight);
        end
