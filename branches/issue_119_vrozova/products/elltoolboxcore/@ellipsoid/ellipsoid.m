        ...
        function [ellMat] = ellipsoid(varargin)
            ...
            if nReg == 0
                ...
                if nReg == 1
                    ...
                    shDimsVec(1:nShDims) = size(shMatArray);
                    checkmultvar(...
                        @(x,y)(x==2&&y==2)||x==y+1, 2, shDimsVec(2), centDimsVec(2),...
                        'errorTag','wrongInput',...
                        'errorMessage', ['centerVec and shapeMat matrix must ',...
                        'differ in dimensionality by 1.']);
%line 245 was changed from: @(x,y)(x==2&&y==2)||x==y+1, 2, nShDims, nCentDims,...
%centDimsVec(1:nCentDims) = size(centVecArray);
%shDimsVec(1:nShDims) = size(shMatArray);
                    checkmultvar(...
                        @(x,y)all(x==y), 2, centDimsVec(3:end),...
                        shDimsVec(3:end), 'errorTag','wrongInput',...
                        'errorMessage',...
                        'additional dimensions must agree');
%line 253 was changed from: @(x,y)all(x==y), 2, centDimsVec(2:end),...
