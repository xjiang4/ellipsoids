...
%            System Analysis Department 2013 $
    ...
    methods
        ...
        function self = testIsContainedInIntersection (self)
            ...
            test1EllArray = self.createObjectArray(array1Size, @(varargin)self.ellFactoryObj.create(varargin{:}), ...
                [2; 1], [4, 1; 1, 1], 2);
            array2Size = [1, 1];
            ...
            test2EllArray = self.createObjectArray(array2Size, @(x)self.ellFactoryObj.create(eye(x)), ...
                3, 1, 1);
            test2EllArray(1, 2, 1, 1, 1, 3, 1) = self.ellFactoryObj.create([2; 1; 0], ...
                myMat);
            testCorrect(1, 'i', 1);
            testCorrect(1, 'u', 0);
...
switch flag
    case 1
        ...
        my1Ell = self.ellFactoryObj.create(diag( 4 * ones(1, 10)));
        my2Ell = self.ellFactoryObj.create(eye(10));
        myEllArray = self.createObjectArray(arraySize, @(x)self.ellFactoryObj.create(eye(x)), ...
            10, 1, 1);
        myMat = eye(10);
        ansEllMat = diag(13 ^ 2 * ones(1, 10));
        ansEllVec = self.createObjectArray([1, 10], @(varargin)self.ellFactoryObj.create(varargin{:}), ...
            ansEllMat, 1, 1);
        varargout{1} = my1Ell;
        varargout{2} = my2Ell;
    ...
    case 2
        ...
        my1Ell = self.ellFactoryObj.create(10 * ones(7, 1), diag(9 * ones(1, 7)));
        my2Ell = self.ellFactoryObj.create(-3 * ones(7, 1), diag([4, ones(1, 6)]));
        myEllArray = self.createObjectArray(arraySize, @(x)self.ellFactoryObj.create(eye(x)), ...
            7, 1, 1);
        myMat = [eye(7), -eye(7)];
        ansEaEllMat = diag([13 ^ 2, 13 * 16 * ones(1, 6)]);
        ansEaEllVec = self.createObjectArray([1, 2], @(varargin)self.ellFactoryObj.create(varargin{:}), ...
            13 * ones(7, 1), ansEaEllMat, 2);
        ansIaEllMat = diag([13 ^ 2, (realsqrt(2.5) + 12) ^ 2 * ones(1, 6)]);
        ansIaEllVec = self.createObjectArray([1, 2], @(varargin)self.ellFactoryObj.create(varargin{:}), ...
    ...
    case 3
        ...
        my1Ell = self.ellFactoryObj.create(eye(1));
        my2Ell = self.ellFactoryObj.create(1, 0.25);
        myEllArray = self.createObjectArray(arraySize, @(x)self.ellFactoryObj.create(eye(x)), ...
            1, 1, 1);
        myMat = [1, -1];
        ansEllMat = diag(6.5 ^ 2);
        ansEllVec = self.createObjectArray([1, 2], @(varargin)self.ellFactoryObj.create(varargin{:}), ...
            -1, ansEllMat, 2);
        varargout{1} = my1Ell;
        varargout{2} = my2Ell;
        varargout{3} = myEllArray;
    ...
    case 5
        ...
        myEllArray = self.createObjectArray(arraySize, @(x)self.ellFactoryObj.create(eye(x)), ...
            7, 1, 1);
        myEllArray(1, 2, 1, 3, 2, 1) = self.ellFactoryObj.create(5 * ones(7, 1), ...
            diag(9 * ones(1, 7)));
        myMat = [eye(7), -eye(7)];
        ansEllMat = diag(14 ^ 2 * ones(1, 7));
        ansEllVec = self.createObjectArray([1, 14], @(varargin)self.ellFactoryObj.create(varargin{:}), ...
    ...
    case 6
        ...
        myEllArray = self.createObjectArray(arraySize, @(x)self.ellFactoryObj.create(eye(x)), ...
            1, 1, 1);
        myEllArray(1, 1, 1, 1, 1, 2, 1, 1, 1) = self.ellFactoryObj.create(-1, 0.25);
        myMat = [1, -1];
    ...
    case 7
        ...
        myEllArray = self.createObjectArray(arraySize, @(x)self.ellFactoryObj.create(eye(x)), ...
            10, 1, 1);
        myEll = self.ellFactoryObj.create(diag( 4 * ones(1, 10)));
        myMat = eye(10);
    ...
    case 8
        arraySize = [1, 2, 1, 3, 2, 1];
        myEllArray = self.createObjectArray(arraySize, @(varargin)self.ellFactoryObj.create(varargin{:}), ...
            -3 * ones(7, 1), diag([4, ones(1, 6)]), 2);
        myEll = self.ellFactoryObj.create(10 * ones(7, 1), diag(9 * ones(1, 7)));
        myMat = [eye(7), -eye(7)];
        ansEaEllMat = diag([21 ^ 2, 9 ^ 2 ...
            * ones(1, 6)]);
        ansEaEllVec = self.createObjectArray([1, 14], @(varargin)self.ellFactoryObj.create(varargin{:}), ...
            -46 * ones(7, 1), ansEaEllMat, 2);
        ans1IaEllMat = diag( 0.875 * 12 ^ 2 * [ 4, ones(1, 6)] - 63);
        ans2IaEllMat = diag( 0.75 * 12 ^ 2 * [ 4, ones(1, 6)] - 27);
        ansIaEllVec = self.createObjectArray([1, 14], @(varargin)self.ellFactoryObj.create(varargin{:}), ...
            -46 * ones(7, 1), ans2IaEllMat, 2);
        ansIaEllVec(1) = self.ellFactoryObj.create(-46 * ones(7, 1), ans1IaEllMat);
        ansIaEllVec(8) = self.ellFactoryObj.create(-46 * ones(7, 1), ans1IaEllMat);
        varargout{1} = myEllArray;
        varargout{2} = myEll;
        varargout{3} = myMat;
    ...
    case 9
        ...
        myEll = self.ellFactoryObj.create(1, 0.25);
        myEllArray = self.createObjectArray(arraySize, @(x)self.ellFactoryObj.create(eye(x)), ...
            1, 1, 1);
        myMat = [1, -1];
...
function [varargout] = createTypicalHighDimArray(self, flag)
switch flag
    case 1
        arraySize = [1, 1, 1, 1, 1, 3, 1, 1, 3, 1];
        my1Ell = self.ellFactoryObj.create(eye(100));
        my2Ell = self.ellFactoryObj.create(ones(100, 1), diag( 0.25 * ones(1, 100)));
        myEllArray = self.createObjectArray(arraySize, @(x)self.ellFactoryObj.create(eye(x)), ...
            100, 1, 1);
        myMat = [eye(5); zeros(95, 5)];
        ansEllMat = diag(9.5 ^ 2 * ones(1, 100));
        ansEllVec = self.createObjectArray([1, 5], @(varargin)self.ellFactoryObj.create(varargin{:}), ...
    ...
    case 3
        ...
        myEllArray = self.createObjectArray(arraySize, @(varargin)self.ellFactoryObj.create(varargin{:}), ...
            ones(100, 1), diag( 0.25 * ones(1, 100)), 2);
        myEll = self.ellFactoryObj.create(eye(100));
        myMat = [eye(5); zeros(95, 5)];
...
function compareForMinkFunc(self, func, nArg, ansEllVec, firstArg, secondArg, thirdArg, ...
    fourthArg)
switch nArg
    case 2
...
function objectArray = createObjectArray(self, arraySize, func, firstArg, ...
    secondArg, nArg)
...
testError(13);
    function testCorrect(isnHighDim, flag)
        ...
        if isMinksumEa
            self.compareForMinkFunc(@minksum_ea, 2, ansEllVec, ...
                myEllArray, myMat);
        else
            self.compareForMinkFunc(@minksum_ia, 2, ansEllVec, ...
                myEllArray, myMat)
        end
    end
    function testError(flag)
        [test1EllArray, ~, errorStr] = self.createTypicalArray(flag);
        if isMinksumEa
            self.runAndCheckError...
...
testError(13);
    function testCorrect(isnHighDim, flag)
        ...
        if isMinkmpEa
            self.compareForMinkFunc(@minkmp_ea, 4, ansEllVec, my1Ell, my2Ell, ...
                myEllArray, myMat);
        else
            self.compareForMinkFunc(@minkmp_ia, 4, ansEllVec, my1Ell, my2Ell, ...
                myEllArray, myMat);
        end
    end
    function testError(flag)
        ...
        if isMinkmpEa
            self.runAndCheckError...
                ('test1EllArray.minkmp_ea(self.ellFactoryObj.create(eye(3)), test2EllArray, eye(3))', ...
                'wrongInput');
            self.runAndCheckError...
                ('self.ellFactoryObj.create(eye(3)).minkmp_ea(test1EllArray, test2EllArray, eye(3))', ...
                'wrongInput');
            if (flag ~= 10)
                self.runAndCheckError...
                    ('self.ellFactoryObj.create(eye(3)).minkmp_ea(self.ellFactoryObj.create(eye(3)), test1EllArray, eye(3))', ...
                    errorStr);
            end
        else
            self.runAndCheckError...
                ('test1EllArray.minkmp_ia(self.ellFactoryObj.create(eye(3)), test2EllArray, eye(3))', ...
                'wrongInput');
            self.runAndCheckError...
                ('self.ellFactoryObj.create(eye(3)).minkmp_ia(test1EllArray, test2EllArray, eye(3))', ...
                'wrongInput');
            if (flag ~= 10)
                self.runAndCheckError...
                    ('self.ellFactoryObj.create(eye(3)).minkmp_ia(self.ellFactoryObj.create(eye(3)), test1EllArray, eye(3))', ...
                    errorStr);
            end
        end
    end
...
testError(13);
    function testCorrect(isnHighDim, flag)
        ...
        if isMinkpmEa
            self.compareForMinkFunc(@minkpm_ea, 3, ansEllVec, myEllArray, ...
                myEll, myMat);
        else
            self.compareForMinkFunc(@minkpm_ia, 3, ansEllVec, myEllArray, ...
                myEll, myMat);
        end
    end
    function testError(flag)
        ...
        if (flag == 10) || (flag == 13)
            if isMinkpmEa
                self.runAndCheckError...
                    ('test1EllArray.minkpm_ea(self.ellFactoryObj.create(eye(3)), eye(3))', ...
                    errorStr);
            else
                self.runAndCheckError...
                    ('test1EllArray.minkpm_ia(self.ellFactoryObj.create(eye(3)), eye(3))', ...
                    errorStr);
            end
        else
            if isMinkpmEa
