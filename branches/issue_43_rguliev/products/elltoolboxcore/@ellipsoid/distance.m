...
ellipsoid.checkIsMe(E,'errorMessage',...
    'first argument must be ellipsoid or array of ellipsoids.');
...
checkmultvar('(x1==1)||(x2==1)||(x1==x2)',2,nEllObj,nVec,...
    'errorMessage','number of ellipsoids does not match the number of vectors.');
%
dimsMat = dimension(ellObjArray);

checkmultvar('x1(:)==x2',2,dimsMat,kSize,...
    'errorMessage','ellipsoids must be of the same dimension that vector.');
...
checkmultvar('(x1==1)||(x2==1)||((x3==x4)&&(x5==x6))',...
    6,nEllObj1,nEllObj2,mSize1,mSize2,kSize1,kSize2,...
...
checkmultvar('all(x1(:)==x1(1)) && all(x2(:)==x2(1))',2,dims1,dims2,...
...
if (t1 > 1) && (t2 > 1)
    d = arrayfun(@(x,y) l_hpdistSingle(x, y, flag),E,X);
elseif (t1 > 1)
    d = arrayfun(@(x) l_hpdistSingle(x, X, flag),E);
else
    d = arrayfun(@(x) l_hpdistSingle(E, x, flag),X);
end

status = [];

    function d = l_hpdistSingle(E, X, flag)
        [v, c] = parameters(X);
        if c < 0
            c = -c;
            v = -v;
